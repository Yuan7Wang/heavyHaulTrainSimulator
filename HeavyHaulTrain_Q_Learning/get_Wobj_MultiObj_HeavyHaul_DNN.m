function W_obj = get_Wobj_MultiObj_HeavyHaul_DNN(nHiddens, rangS, rangV, detaS, detaV, nTiling, nl, nPosNotch, nNegNotch)
%% state_coding
...
W_obj.nG = size(W_obj.G, 1); % unit m
...
W_obj.choose_next_action_Multi = @choose_next_action_Multi;
...
%% 
function action_set = choose_next_action_Multi(W_OBJ, current_state, current_action, epsilon, reward_weigh)
%% coding 

current_ind_sv = coding_index_transform(W_OBJ{1}, current_state);
action_set = zeros(size(current_action));
...
for i = 1:size(action_set, 2)
    ...
    current_sv_coding = current_ind_sv(:, i);
    notch = current_action(:, i);
    [notch_set, N_ind] = get_possible_notch_set(W_OBJ{1}, notch);
    
    Val = zeros(length(reward_weigh), W_OBJ{1}.nG);
    if rand > epsilon    % ------ greedy
        for obj = 1:length(reward_weigh)
            hidden_out_ = cal_hidden_output(W_OBJ{obj}, current_sv_coding);
            Val(obj, :) = cal_final_output(W_OBJ{obj}, hidden_out_{end}, N_ind);
        end
        % 多目标加权和
        vals = reward_weigh*Val;
