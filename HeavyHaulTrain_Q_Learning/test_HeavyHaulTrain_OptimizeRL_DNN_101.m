...
% function HeavyHaulTrain_OptimizeRL_DNN(save_data_string, TotalLength, ...
%     total_step_max, learning_rate, reward_weigh, flag_up_or_down_ramp, flag_speed_limit)
...
% reward_weigh = [0.3 2/1e6 5/1e6*1.5]*1e0;   % 速度 车钩力， 总能耗 % ------------- weight 多个目标之间的相互权重大小
...
disp_cycle = [50 200 200];    % 状态显示栏基本周期参数
...
%     W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, Qloc);
    ...
    bool_restart = bool_terminate | countStepNe >= maxStep  ...
        | V_observation - lowSpeedLim < -2  | V_observation - max(Lspeed) > 5;
    ...
    uniform_ind = ceil(rand(replay_batch, 1)*replay_flag);
    W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, QLOC(uniform_ind, :));
    
end
toc

% save(save_data_string)
