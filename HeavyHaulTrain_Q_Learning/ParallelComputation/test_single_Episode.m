...
%     epsilon  = epsilons(etr);
    ...
    AirBrkNotch = -zeros(1, Ne);       % 空气制动，-2, -1, 0;
    AirBrkNotch = AirBrkNotch + ActionSet(action, 3)';
    ...
    for itr = 1:length(tvec)
        ...
        % ---------------------------------------------- 空气制动力
%         air_timer = air_timer+dt;     % 时钟增加一次
%         air_timer_ = air_timer_+dt;   % 时钟增加一次
%         air_time_d = air_timer - airTimeDelay;          % 空气制动时间差，大于0表示已经传达，小于0表示尚未到达
%         air_time_d_ = air_timer_ - airTimeDelay;          % 空气制动时间差，大于0表示已经传达，小于0表示尚未到达
%         [currentF, F_target] = GetAirBrakeF(currentF, F_target, ...
%             AirBrkNotch, AirBrkNotch_, air_time_d, air_time_d_, Abr, dt);  % 空气制动力
%         
%         tmp = currentF;
%         tmp(X(~carType, :) <= 0) = 0;
%         U(~carType, :) = tmp  * 1e3;   % 空气制动力
...
%         F_w(itr, :) = sum(currentF.^2);
        ...
        if rem(itr, floor(detaT/dt)) == 0   % 每 10 次 迭代做一次操作
            ...
            AirBrkNotch = AirBrkNotch + ActionSet(action, 3)';       % 空气制动，-2, -1, 0; 
            AirBrkNotch(AirBrkNotch > 0) = 0;
            AirBrkNotch(AirBrkNotch < -2) = -2;
            
            
            %%
            tmpBo = ones(Nw, 1)*(AirBrkNotch_ ~= AirBrkNotch) == 1;  % 级位发生变化，再重置计时器
