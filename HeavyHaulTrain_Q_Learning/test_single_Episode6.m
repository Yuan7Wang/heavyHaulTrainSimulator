...
%     
    ...
    MotorNotch = zeros(Nl, Ne) + 5;     % 每个机车有独立的级位
...
%         F_c(itr, :) = sum(F_c_.^2);
        ...
        if rem(itr, floor(detaT/dt)) == 0   % 每 10 次 迭代做一次操作
            ...
            for i = 1:Ne
                if bo_tmp(i)      % epsilon - greedy
                    [~, action_(i)] = max(Q(s_tmp(i), v_tmp(i), MotorNotch(1, i)+13, MotorNotch(2, i)+13, :));
                else
                    action_tmp = find(~isnan(Q(s_tmp(i), v_tmp(i), MotorNotch(1, i)+13, MotorNotch(2, i)+13, :)));
            ...
            for i = 1:Ne
                    Q(s_tmp_(i), v_tmp_(i), MotorNotch_(1, i)+13, MotorNotch_(2, i)+13, action(i)) = ...
                        Q(s_tmp_(i), v_tmp_(i), MotorNotch_(1, i)+13, MotorNotch_(2, i)+13, action(i))...
                        + alpha*(R(i) + gamma*(Q(s_tmp(i), v_tmp(i), MotorNotch(1, i)+13, MotorNotch(2, i)+13, action(i))) ...
                        - Q(s_tmp_(i), v_tmp_(i), MotorNotch_(1, i)+13, MotorNotch_(2, i)+13, action(i))); 
            end
