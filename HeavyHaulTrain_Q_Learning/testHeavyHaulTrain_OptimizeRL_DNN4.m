% testHeavyHaulTrain_OptimizeRL_DNN4.m
...
hHpeed_Limits = [65 80 95]/3.6;
lowSpeedLim = 15/3.6;
speed_ChangePoint = TotalLength*[3/10  6/10];
...
nHiddens = [128  ]; %#ok<NBRAK>
% nHiddens = [100 200 300];  
mini_batch = 2^10;
W_obj = get_Wobj_4HeavyHaul_DNN3(nHiddens, maxS, maxV, detaS, detaV, nTiling, nl, nPosNotch, nNegNotch, mini_batch);
...
%% 
current_ind_SomeState1 = W_obj.coding_index_transform(W_obj, [5000 30/3.6]');
current_ind_SomeState2 = W_obj.coding_index_transform(W_obj, [10000 30/3.6]');  
figure(10003);clf
subplot(121);
...
hActionVal1 = contourf(W_obj.Ax, W_obj.Ay, val, 20);
...
val = reshape(W_obj.cal_final_output(W_obj, hidden_out_{end}), 25, []);
hActionVal2 = contourf(W_obj.Ax, W_obj.Ay, val, 20);
colorbar
% view(-48, 67)
...
while total_step_count <= total_step_max 
    ...
    if rem(total_step_count, 10) == 0  % update_plot 
        time_consumed = toc;
...
%     Lspeed(~bo_tmp) = hHpeed_Limits(2);
    ...
    speed_r = higSpeed_r(X(1, :) - min(Lspeed)) + lowSpeed_r(X(1, :) - lowSpeedLim);
    R = R + speed_r;
...
%     for iloop = 1:1
        uniform_ind = ceil(rand(mini_batch, 1)*replay_flag);
%         W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, QLOC(uniform_ind, :));
        W_obj = W_obj.update_ActionFunction_half_batch(W_obj, W_obj_copy, QLOC(uniform_ind, :));
%     end
end
