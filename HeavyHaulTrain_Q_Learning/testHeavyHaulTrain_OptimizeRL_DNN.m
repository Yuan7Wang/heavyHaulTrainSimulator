...
TotalLength = 12e3;
initial_posRange = TotalLength*0.6; 
% initial_position = rand*initial_posRange;
% initial_speed = 60/3.6;
% initial_notch = 7;

hHpeed_Limits = [85 65 95]/3.6;
lowSpeedLim = 15/3.6;
speed_ChangePoint = TotalLength*[3/10 5/10];
...
nHiddens = [500  ]; %#ok<NBRAK>
% nHiddens = [200 200 200];  
W_obj = get_Wobj_4HeavyHaul_DNN(nHiddens, maxS, maxV, detaS, detaV, nTiling, nl, nPosNotch, nNegNotch);
...
rampList = [-10000 0; 0 0.00; 1500 -0.00; 3000 0; 10000 0; 20000 0; 30000 0; 50000 0];
...
gamma = 0.98;
alpha = 1e-3/nTiling/5;

W_obj.gamma = gamma;
W_obj.alpha = alpha;

epsilons = [linspace(4e-1, 1e-2, total_step_max) 0];
epsilon = epsilons(1);
...
higSpeed_r = @(dv) (a*(abs(dv)+3).^3 + b).*(dv > 0);
lowSpeed_r = @(dv) (a*(abs(dv)+6).^3 + b).*(dv < 0);
...
%     W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, Qloc);
    ...
    bool_restart = bool_terminate | countStepNe >= maxStep  | X(1, :) - lowSpeedLim < -3  | X(1, :) - max(Lspeed) > 3;
...
%         Lspeed_(~bo_tmp) = hHpeed_Limits(2);
        ...
        initial_notch = ceil(rand(1, tmp_ind)*10)-1;
