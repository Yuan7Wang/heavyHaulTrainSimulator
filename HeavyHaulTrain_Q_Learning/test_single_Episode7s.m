...
Qloc = zeros(in_epi_number, 4, Ne);    % 每一步的 state-action
...
%             end
        ...
        if rem(itr, floor(detaT/dt)) == 0   % 每 10 次 迭代做一次操作
            ...
            %% 记录状态-行为标号，以及对应的回报值
            for i = 1:Ne
                Qloc(count, :, i) = [s_tmp_(i), v_tmp_(i), MotorNotch_(1, i)+13, action(i)];
            end
            Rs_record(count, :) = R;
...
%             end
    ...
    for i = 1:Ne
        ...
        for jtr = 1:count
            n_tmp = Qc(Qloc(jtr, 1, i), Qloc(jtr, 2, i), Qloc(jtr, 3, i), Qloc(jtr, 4, i));
            v_tmp = Q(Qloc(jtr, 1, i), Qloc(jtr, 2, i), Qloc(jtr, 3, i), Qloc(jtr, 4, i));
            Q(Qloc(jtr, 1, i), Qloc(jtr, 2, i), Qloc(jtr, 3, i), Qloc(jtr, 4, i)) = (v_tmp*n_tmp+R_tmp_(jtr))/(n_tmp+1);
            Qc(Qloc(jtr, 1, i), Qloc(jtr, 2, i), Qloc(jtr, 3, i), Qloc(jtr, 4, i)) = n_tmp+1;
        end
    end
