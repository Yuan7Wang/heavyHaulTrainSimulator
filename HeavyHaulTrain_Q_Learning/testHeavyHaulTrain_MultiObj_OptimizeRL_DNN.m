% testHeavyHaulTrain_MultiObj_OptimizeRL_DNN.m

% clear; clc

flag_up_or_down_ramp = 0;    % 0 表示上山，1 表示下山
flag_speed_limit = 0;        % 0 表示 limit 1，1 表示 limit 2
% function testHeavyHaulTrain_MultiObj_OptimizeRL_DNN(flag_up_or_down_ramp, flag_speed_limit)
%% 
...
reward_weigh = [0.3 2/1e6 5/1e6*1.5];   % 车钩力， 总能耗
...
W_obj = get_Wobj_MultiObj_HeavyHaul_DNN(nHiddens, maxS, maxV, detaS, detaV, nTiling, nl, nPosNotch, nNegNotch);
...
rampList_down = [-10000 0; 
    ...
    50000 0; 
    100000 0];
rampList_down(:, 2) = rampList_down(:, 2) * 5;
% 
...
nQloc = 2 + nl + 2 + 1;
Qloc = zeros(Ne, nQloc, 3);    % 每一步的 state-action
local_point = 0;
QLOC = zeros(Ne * replay_buffer_size, nQloc, 3)*NaN;    % 每一步的 state-action
...
%         tmp_Fl = tmp_Fl.*(1+rand(size(tmp_Fl))*0.02-0.01);
    ...
    F_c_max = mean(F_c_max_);
    F_c_min = mean(F_c_min_);
    R_tmp(1, :) = sqrt(mean(F_c.^2));
    ...
    R1 = (V_observation-33)*reward_weigh(1);
    R1(R1 > 0) = 0;
    Lspeed = W_obj.obtain_speed_limit(X([1+Nt Nt*2], :) + pos_train, speed_ChangePoint, hHpeed_Limits);
    speed_r = higSpeed_r(V_observation - min(Lspeed)) + lowSpeed_r(V_observation - lowSpeedLim);
    R1 = R1 + speed_r;
    
    R2 = - reward_weigh(2)*(R_tmp(1, :));
    R3 = - reward_weigh(3)*(R_tmp(2, :));
    % ------------- 分别记录各项 reword 的值
    Recorder_r(total_step_count, :) = [mean(R1), mean(R2), mean(R3)];
    
    R1 = R1.*(1 + noise_sensor(total_step_count, :));
    R2 = R2.*(1 + noise_sensor(total_step_count, :));
    R3 = R3.*(1 + noise_sensor(total_step_count, :));
    
    bool_terminate = s_tmp >= TotalLength;
    R1(bool_terminate) = -R1(bool_terminate);  % 转化为正数回报以标识终点回报
    R2(bool_terminate) = -R2(bool_terminate);  % 转化为正数回报以标识终点回报
    R3(bool_terminate) = -R3(bool_terminate);  % 转化为正数回报以标识终点回报
    ...
    for i = 1:Ne   % 记录，并更新Q值  
        Qloc(i, :, 1) = [s_tmp_(i), v_tmp_(i), MotorNotch(:, i)', s_tmp(i), v_tmp(i), R1(i)]; 
        Qloc(i, :, 2) = [s_tmp_(i), v_tmp_(i), MotorNotch(:, i)', s_tmp(i), v_tmp(i), R2(i)]; 
        Qloc(i, :, 3) = [s_tmp_(i), v_tmp_(i), MotorNotch(:, i)', s_tmp(i), v_tmp(i), R3(i)]; 
    end
    % 先临时更新一次，数据至少使用一次
%     W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, Qloc);
%     W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, Qloc);
    for obj = 1:3
        W_OBJ{obj} = W_obj.update_ActionFunction(W_OBJ{obj}, W_OBJ_copy{obj}, Qloc(:, :, obj));
    end
...
%     actions = W_obj.choose_next_action(W_obj, [s_tmp; v_tmp], MotorNotch, epsilon);
    ...
    for obj = 1:3
        W_OBJ{obj} = W_obj.update_ActionFunction(W_OBJ{obj}, W_OBJ_copy{obj}, QLOC(uniform_ind, :, obj));
    end
end
