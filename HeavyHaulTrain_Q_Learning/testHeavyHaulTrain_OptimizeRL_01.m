% testHeavyHaulTrain_OptimizeRL_01.m
...
hHpeed_Limits = [80/3.6 65/3.6 ];
lowSpeedLim = 20/3.6;
speed_ChangePoint = TotalLength*5/10;
...
detaS = 400; % unit m
detaV = 20;  % unit km/h
maxV = [0 100];  % unit km/h
maxS = [0 TotalLength]; % unit m
nTiling = 50;  % number of tiling 
nAction = (nPosNotch + nNegNotch + 1)^nl;
W_obj = get_Wobj_4HeavyHaul_linear(maxS, maxV, detaS, detaV, nTiling, nl, nPosNotch, nNegNotch);
...
% rampList = [-10000 0; 0 0.01; 1500 -0.01; 3000 0; 10000 0; 20000 0; 50000 0];
rampList = [-10000 0; 0 0.00; 1500 -0.00; 3000 0; 20000 0; 50000 0; 80000 0; 120000 0];
...
gamma = 0.98;
alpha = 8e-2/nTiling/5;

W_obj.gamma = gamma;
W_obj.alpha = alpha;

epsilons = [linspace(2e-1, 1e-2, total_step_max) 0];
epsilon = epsilons(1);
...
reward_weigh = [1/1e9 1/1e9]*5e-1;   % 车钩力， 总力的积分
...
figure(10001);clf
hold on; box on;
plot([0 speed_ChangePoint speed_ChangePoint TotalLength], hHpeed_Limits([1 1 2 2])*3.6, 'k', 'linewidth', 2);
plot([0  TotalLength], lowSpeedLim([1 1])*3.6, 'k', 'linewidth', 2);
set(gca, 'xlim', [0 TotalLength], 'ylim', [0 max(hHpeed_Limits)*3.6+20])

h_traj = zeros(2, Ne);
for i = 1:Ne
%     h_traj(1, i) = plot(phase_buffer(1, 1, i), phase_buffer(1, 2, i)*3.6, 'ko');
...
v = linspace(W_obj.rangV(1), W_obj.rangV(2), 100);
[Ms, Mv] = meshgrid(s, v);
stateSet = [Ms(:), Mv(:)]';
...
% Val = W_obj.max_Q_value_of_State(W_obj, current_ind_sv);
for itr = 1:numel(Ms)
    Val(itr) = sum(sum(W_obj.W(N_ind, current_ind_sv(:, itr))));
...
current_ind_SomeState1 = W_obj.coding_index_transform(W_obj, [3500 75/3.6]');
current_ind_SomeState2 = W_obj.coding_index_transform(W_obj, [5000 50/3.6]');  
figure(10003);clf
subplot(121);
hActionVal1 = mesh(W_obj.Ax, W_obj.Ay, reshape(W_obj.cal_final_output(W_obj, current_ind_SomeState1), 25, []));
view(-12, 75)
set(gca, 'zdir', 'reverse')
subplot(122);
hActionVal2 = mesh(W_obj.Ax, W_obj.Ay, reshape(W_obj.cal_final_output(W_obj, current_ind_SomeState2), 25, []));
view(-12, 75)
set(gca, 'zdir', 'reverse')
...
%         
        set(hActionVal1, 'zdata', reshape(W_obj.cal_final_output(W_obj, current_ind_SomeState1), 25, []));
        set(hActionVal2, 'zdata', reshape(W_obj.cal_final_output(W_obj, current_ind_SomeState2), 25, []));
        drawnow;
        pause(0.00001);
    end
...
%     W_obj = W_obj.update_ActionFunction(W_obj, W_obj_copy, Qloc);
    W_obj = W_obj.update_ActionFunction(W_obj, Qloc);
    
    %% 如果有终止发生，生成新的随机状态，开始探索
    
    bool_restart = bool_terminate | countStepNe >= maxStep  | X(1, :) - lowSpeedLim < -4  | X(1, :) - max(Lspeed) > 4;
    if any(bool_restart)   % 如果有终止发生, 生成随机种子，重新开始
        ...
        initial_notch = ceil(rand(nl, tmp_ind)*10)-1;
    ...
    uniform_ind = ceil(rand(replay_batch*5, 1)*replay_flag);
    W_obj = W_obj.update_ActionFunction(W_obj,  QLOC(uniform_ind, :));
