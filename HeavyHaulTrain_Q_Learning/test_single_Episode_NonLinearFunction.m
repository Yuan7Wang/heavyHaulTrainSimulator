...
reward_weigh = [1/1e7 1/1e7 0 1/1e4*8]*1e-0;
...
%     X(1:Nt, :) = ones(Nt, Ne)*initial_speed;     % 初速度
    ...
    actions = get_action_by_Wobj_Nonlinear(W_obj, [s_tmp; v_tmp], MotorNotch, epsilon);
...
%         Qloc_ = reshape(permute(Qloc, [1 3 2]), [], nQloc);
        W_obj = update_ActionFunction_Nonlinear(W_obj, W_obj_copy, Qloc(end:-1:1, :), alpha);
        ...
        W_obj = update_ActionFunction_Nonlinear(W_obj, W_obj_copy, QLOC(uniform_ind, :), alpha);
...
vals = W_obj.cal_final_output(W_obj, W_obj.cal_hidden_output(W_obj, current_ind_sv), 1:W_obj.nNotch^2);
